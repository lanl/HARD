    ...
    s.executor().forall(k, (m.template cells<ax::z, dm::quantities>())) {
      for(auto j : m.template cells<ax::y, dm::quantities>()) {
        for(auto i : m.template cells<ax::x, dm::quantities>()) {
          ...
          const auto y0{m.template head<ax::y>(j)};
          const auto y1{m.template tail<ax::y>(j)};
          const auto z0{m.template head<ax::z>(k)};
          const auto z1{m.template tail<ax::z>(k)};
          ...
          momentum_density(i, j, k).x() = mass_density(i, j, k) * ux;
          momentum_density(i, j, k).y() = mass_density(i, j, k) * uy;
          momentum_density(i, j, k).z() = mass_density(i, j, k) * uz;
          const double e = util::find_sie(eos, mass_density(i, j, k), p0);
          total_energy_density(i, j, k) =
            mass_density(i, j, k) * e +
