...
template<template<std::size_t> typename S, std::size_t D, class Policy>
struct control_base : flecsi::run::control_base {

  using control_policy = Policy;
...
struct control_policy : spec::control_base<S, D, control_policy<S, D>> {

  using base_t = spec::control_base<S, D, control_policy<S, D>>;
  using control_points_enum = cp;
  template<auto P>
  using point = typename base_t::template point<P>;
  template<auto Fn, class... Ps>
  using cycle = typename base_t::template cycle<Fn, Ps...>;
  template<class... TT>
  using list = typename base_t::template list<TT...>;
  ...
  control_policy(double t0,
    ...
    std::size_t log_frequency,
    std::size_t output_frequency)
    : base_t(t0, tf, max_steps, cfl, max_dt, log_frequency, output_frequency) {}

  using control_points = list<point<cp::initialize>,
    cycle<base_t::cycle_control,
      point<cp::rhs>,
      point<cp::rk_stage_1>,
      point<cp::rk_stage_2>,
      point<cp::update_dt>,
