...
struct RK {
  ...
  const auto & total_energy_density() const {
    return std::get<1>(f);
  }

  auto & momentum_energy_density() {
    return std::get<2>(f);
  }

  const auto & momentum_energy_density() const {
    return std::get<2>(f);
  }

  auto operator()(const mesh<D>::ptr & s) {
    return std::make_tuple(
      std::get<0>(f)(*s), std::get<1>(f)(*s), std::get<2>(f)(*s));
  }
